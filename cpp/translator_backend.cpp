...
{
    setStatus(uiTr(QStringLiteral("status_check_python")));
    QTimer::singleShot(0, this, [this]() {
        DocumentLoader::ensureReady();
        const ExtractRuntimeInfo info = DocumentLoader::probe();
        if (info.ready)
            setStatus(uiTr(QStringLiteral("status_ready")));
        else if (!DocumentLoader::activePythonPath().isEmpty())
            setStatus(uiTrArgs(QStringLiteral("status_python_path"),
                               {DocumentLoader::activePythonPath()}));
        else
...
void TranslatorBackend::refreshHardware()
{
    ...
    if (GlobalMemoryStatusEx(&memInfo)) {
        ...
        m_ramLabel = uiTr(QStringLiteral("hw_ram_usage"))
                         .arg(QString::number(used, 'f', 1), QString::number(m_ramTotalGb, 'f', 1));
    } else {
        m_ramLabel = uiTr(QStringLiteral("hw_ram_unknown"));
    }
#else
    m_ramLabel = uiTr(QStringLiteral("hw_ram_unknown"));
#endif
    m_vramLabel = uiTr(QStringLiteral("hw_vram_unknown"));
    m_vramTotalGb = 0;
...
void TranslatorBackend::updateModelInfo(const QString& modelId)
{
    ...
    if (!speedFromCatalog.isEmpty()) {
        ...
        m_modelSpeed = uiTr(QStringLiteral("main_speed_tokens")).arg(42);
    } else if (modelId.contains(QStringLiteral("4b"))) {
        m_modelSpeed = uiTr(QStringLiteral("main_speed_tokens")).arg(85);
    } else if (modelId.contains(QStringLiteral("deepseek"))) {
        m_modelSpeed = uiTr(QStringLiteral("main_speed_tokens")).arg(120);
    } else {
        m_modelSpeed = uiTr(QStringLiteral("main_speed_tokens")).arg(95);
    }
    ...
    if (!qualityFromCatalog.isEmpty()) {
        ...
        m_modelQuality = uiTr(QStringLiteral("main_quality_very_high"));
    } else {
        m_modelQuality = uiTr(QStringLiteral("main_quality_high"));
    }
    ...
    if (hasVramReq) {
        if (m_vramTotalGb > 0) {
            m_vramLabel = uiTr(QStringLiteral("hw_vram_usage"))
                              .arg(QString::number(qMin(m_vramTotalGb, static_cast<double>(reqVram)), 'f', 1),
                                   QString::number(m_vramTotalGb, 'f', 1));
        } else {
            m_vramLabel = uiTr(QStringLiteral("hw_vram_required")).arg(reqVram);
        }
    }
...
{
    const QString localized = localizeStatusMessage(text);
    if (m_status == localized) return;
    m_status = localized;
    emit statusChanged();
}
...
{
    const QString lang = m_appSettings ? m_appSettings->appUiLanguage() : QStringLiteral("ru");
    return AppUiStrings::textArgs(key, lang, args);
}

QString TranslatorBackend::localizeStatusMessage(const QString& message) const
{
    const QString lang = m_appSettings ? m_appSettings->appUiLanguage() : QStringLiteral("ru");
    return AppUiStrings::translateMessage(message, lang);
}

void TranslatorBackend::onAppUiLanguageChanged()
{
    refreshHardware();
    if (!m_model.isEmpty())
        updateModelInfo(m_model);
...
QString TranslatorBackend::saveFileFilter() const
{
    const bool englishUi = m_appSettings && m_appSettings->appUiLanguage() != QStringLiteral("ru");
    return TranslationWorkflow::saveFileFilter(englishUi, m_isPdf && hasTranslatedPdf(), m_workflowRoundTrip);
}
...
void TranslatorBackend::buildTranslatedPdfAsync()
{
    ...
    if (!ensurePdfWorkCopy()) {
        ...
        setStatus(uiTr(QStringLiteral("status_pdf_copy_failed_after_translate")));
        if (m_etaTimer)
            m_etaTimer->stop();
        setBusy(false);
    ...
    setStatus(uiTr(QStringLiteral("status_building_translated_pdf")));
    ...
    QTimer::singleShot(0, this, [this, sourcePath, layout, translatedPages, outputPath, workCopyPath]() {
        ...
        if (buildResult.ok) {
            ...
            m_progress = 100;
            emit progressChanged();
            setStatus(uiTrArgs(QStringLiteral("status_translate_done_pdf_pages"),
            ...
            setStatus(buildResult.error.isEmpty()
                          ? uiTr(QStringLiteral("status_translate_done_pdf_not_built"))
                          : uiTrArgs(QStringLiteral("status_translate_done_pdf_error"),
                                     {buildResult.error.left(380)}));
        }
...
void TranslatorBackend::loadFile(const QString& path)
{
    ...
    if (!info.exists() || !info.isFile()) {
        setStatus(uiTrArgs(QStringLiteral("status_file_not_found"), {filePath}));
        return;
    }

    const int generation = ++m_loadGeneration;
    setStatus(uiTr(QStringLiteral("status_loading_extracting")));
    setBusy(true);
...
void TranslatorBackend::applyLoadedDocument(const DocumentLoadResult& doc, const QFileInfo& fileInfo)
{
    ...
    if (isPdf && doc.ok) {
        ...
        if (m_pdfWorkCopyPath.isEmpty()) {
            pdfCopyWarning = uiTrArgs(QStringLiteral("status_pdf_copy_warning"),
                                      {DocumentLoader::pythonDebugLogPath()});
        } else {
            emit pdfPreviewUrlChanged();
        }
    }
    ...
    if (!doc.ok) {
        ...
        const QString pyHint = DocumentLoader::activePythonPath().isEmpty()
            ? QString()
            : QStringLiteral("\n") + uiTrArgs(QStringLiteral("status_python_path"),
                                                {DocumentLoader::activePythonPath()});
        setStatus(doc.error + pyHint + QStringLiteral("\n")
                  + uiTr(QStringLiteral("status_pdf_preview_no_text")));
    } else {
        ...
        if (m_sourceText.trimmed().isEmpty()) {
            ...
            setStatus(uiTr(QStringLiteral("status_no_text_in_doc")));
        } else {
            const QString enc = doc.encoding.isEmpty()
                ? QString()
                : uiTrArgs(QStringLiteral("status_encoding_suffix"), {doc.encoding});
            const QString mode = hasPdfLayout ? uiTr(QStringLiteral("status_mode_pdf_layout"))
                : (m_structuredDocument ? uiTr(QStringLiteral("status_mode_structure"))
                                        : (m_segmentMode ? uiTr(QStringLiteral("status_mode_segments"))
                                                         : QString()));
            setStatus(uiTrArgs(QStringLiteral("status_file_loaded"),
                               {m_workflowTitle,
                                m_workflowPipeline,
                                QString::number(m_pageCount),
                                enc + mode + pdfCopyWarning}));
        }
    }
...
void TranslatorBackend::saveResult(const QString& path)
{
    ...
    if (filePath.isEmpty()) {
        setStatus(uiTr(QStringLiteral("status_no_save_file")));
        return;
    }
    ...
    if (m_isPdf && hasTranslatedPdf()) {
        ...
        if (QFile::copy(m_translatedPdfPath, filePath)) {
            setStatus(uiTrArgs(QStringLiteral("status_pdf_saved"),
                               {QFileInfo(filePath).fileName()}));
            return;
        }
        setStatus(uiTr(QStringLiteral("status_pdf_save_failed")));
        return;
    }
    ...
    if (m_workflowRoundTrip && !m_workflowMeta.isEmpty() && !m_filePath.isEmpty()) {
        ...
        if (exported.ok) {
            setStatus(uiTrArgs(QStringLiteral("status_file_saved"),
                               {QFileInfo(exported.outputPath).fileName()}));
            return;
        }
        if (!exported.error.isEmpty())
            setStatus(uiTrArgs(QStringLiteral("status_export_fallback"),
                               {exported.error.left(180)}));
    }
    ...
    if (!f.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Truncate)) {
        setStatus(uiTr(QStringLiteral("status_file_save_failed")));
        return;
    }
    f.write(m_translatedText.toUtf8());
    f.close();
    setStatus(uiTrArgs(QStringLiteral("status_file_saved"), {QFileInfo(filePath).fileName()}));
}
...
void TranslatorBackend::cancelTranslate()
{
    if (!m_busy) return;
    m_cancelled = true;
    if (m_externalPdfWorker) {
        QMetaObject::invokeMethod(m_externalPdfWorker, "cancel", Qt::QueuedConnection);
        setStatus(uiTr(QStringLiteral("status_cancel_pdf")));
        return;
    }
    ...
    if (m_etaTimer)
        m_etaTimer->stop();
    setBusy(false);
    setStatus(uiTr(QStringLiteral("status_translate_cancelled")));
}

void TranslatorBackend::cleanupExternalPdfTranslateJob()
{
    if (m_externalPdfThread) {
        m_externalPdfThread->quit();
        m_externalPdfThread->wait(5000);
        m_externalPdfThread = nullptr;
    }
    m_externalPdfWorker = nullptr;
}
                                       ...
                                       const QString& targetLang)
{
    ...
    if (combinedSourceText().isEmpty()) {
        if (!m_fileName.isEmpty() && (m_isPdf || m_fileName.endsWith(QStringLiteral(".docx"), Qt::CaseInsensitive)))
            setStatus(uiTr(QStringLiteral("status_no_extracted_text")));
        else
            setStatus(uiTr(QStringLiteral("status_no_text_translate")));
        return;
    }
    if (m_sourceText.trimmed().isEmpty())
    ...
    if (apiKey.trimmed().isEmpty()) {
        setStatus(uiTr(QStringLiteral("status_cloud_api_key")));
        return;
    }
...
void TranslatorBackend::proceedWithTranslate()
{
    ...
    if (m_isPdf && layoutAuto && m_pdfLayout.isEmpty() && !m_filePath.isEmpty()) {
        setBusy(true);
        setStatus(uiTr(QStringLiteral("status_pdf_extract_layout")));

        QTimer::singleShot(0, this, [this]() {
            if (!ensurePdfWorkCopy()) {
                setStatus(uiTr(QStringLiteral("status_pdf_copy_layout_failed")));
                runTranslateJob();
                return;
            }
            ...
            if (layoutResult.ok && !layoutResult.pdfLayout.isEmpty()) {
                ...
                setStatus(uiTrArgs(QStringLiteral("status_pdf_layout_error"),
                                   {layoutResult.error.left(160)}));
            }
...
void TranslatorBackend::runExternalPdfEngine(const QString& engine)
{
    ...
    setStatus(uiTrArgs(QStringLiteral("status_pdf_engine_running"), {engineLabel}));

    if (!ensurePdfWorkCopy()) {
        setStatus(uiTr(QStringLiteral("status_pdf_copy_failed")));
        setBusy(false);
        return;
    }
    ...
    cleanupExternalPdfTranslateJob();

    static const bool metaRegistered = []() {
        qRegisterMetaType<ExternalPdfTranslateResult>("ExternalPdfTranslateResult");
        return true;
    }();
    Q_UNUSED(metaRegistered);

    m_externalPdfThread = new QThread(this);
    m_externalPdfWorker = new ExternalPdfTranslateWorker(req);
    m_externalPdfWorker->moveToThread(m_externalPdfThread);

    connect(m_externalPdfThread, &QThread::started, m_externalPdfWorker,
            &ExternalPdfTranslateWorker::start);
    connect(m_externalPdfWorker, &ExternalPdfTranslateWorker::finished, m_externalPdfThread,
            &QThread::quit);
    connect(m_externalPdfThread, &QThread::finished, m_externalPdfThread, &QObject::deleteLater);
    connect(m_externalPdfWorker, &ExternalPdfTranslateWorker::finished, m_externalPdfWorker,
            &QObject::deleteLater);

    connect(m_externalPdfWorker, &ExternalPdfTranslateWorker::progressUpdated, this,
            [this](const QString& message) {
                static const QRegularExpression percentPattern(QStringLiteral(R"((\d+)%)"));
                const QRegularExpressionMatch match = percentPattern.match(message);
                if (match.hasMatch()) {
                ...
                setStatus(message.left(220));
                updateTranslationEta();
            });

    connect(
        m_externalPdfWorker, &ExternalPdfTranslateWorker::finished, this,
        [this, engineLabel](const ExternalPdfTranslateResult& result) {
            cleanupExternalPdfTranslateJob();

            if (result.ok) {
                m_translatedPdfPath = result.outputPath;
                emit translatedPdfPathChanged();
                m_translatedText = uiTrArgs(QStringLiteral("status_pdf_translated_body"), {engineLabel});
                emit translatedTextChanged();
                m_progress = 100;
                emit progressChanged();
                setStatus(uiTrArgs(QStringLiteral("status_pdf_ready_engine"), {engineLabel}));
                animateWords(m_translatedText);
            } else {
                m_progress = 0;
                emit progressChanged();
                setStatus(result.error.isEmpty()
                              ? uiTr(QStringLiteral("status_external_pdf_failed"))
...
void TranslatorBackend::runTranslateJob()
{
    ...
    emit progressChanged();
    updateTranslationEta();
    setStatus(uiTrArgs(QStringLiteral("status_translate_pages_progress"),
                       {QString::number(m_pageCount)}));
    setBusy(true);
    updateModelInfo(m_model);
                                            ...
                                            bool segmentChunk)
{
    ...
    const int pageNum = chunkIndex < m_chunkPageIndex.size() ? m_chunkPageIndex.at(chunkIndex) + 1
                                                             : m_completedChunks;
    setStatus(uiTrArgs(QStringLiteral("status_translate_progress"),
