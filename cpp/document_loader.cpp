...
QString sanitizeCommandLineForLog(const QString& commandLine)
{
    QString safe = commandLine;
    static const QRegularExpression cloudKeyArg(
        QStringLiteral("--cloud-key(?:=|\\s+)\\S+"),
        QRegularExpression::CaseInsensitiveOption);
    safe.replace(cloudKeyArg, QStringLiteral("--cloud-key ***"));
    static const QRegularExpression openAiKeyEnv(
        QStringLiteral("(OPENAI_API_KEY=)\\S+"),
        QRegularExpression::CaseInsensitiveOption);
    safe.replace(openAiKeyEnv, QStringLiteral("\\1***"));
    return safe;
}
                        ...
                        bool processStarted)
{
    ...
    stream << QStringLiteral("cmd: ") << sanitizeCommandLineForLog(commandLine) << QLatin1Char('\n');
                ...
                const QHash<QString, QString>& extraEnv = {})
{
    ...
    DocumentLoader::configurePythonProcess(proc, extraEnv);
                     ...
                     const QHash<QString, QString>& extraEnv = {})
{
    ...
    const bool started = runProcess(proc, timeoutMs, stdoutOut, stderrOut, extraEnv);
...
void DocumentLoader::configurePythonProcess(QProcess& proc,
                                            const QHash<QString, QString>& extraEnv)
{
...
void DocumentLoader::logPythonRun(const QString& tag,
                                  const QString& commandLine,
                                  const QByteArray& stdoutOut,
                                  const QByteArray& stderrOut,
                                  int exitCode,
                                  bool processStarted)
{
    appendPythonRunLog(tag, commandLine, stdoutOut, stderrOut, exitCode, processStarted);
}
...
ExternalPdfTranslateCommand DocumentLoader::buildExternalPdfTranslateCommand(
    const ExternalPdfTranslateRequest& request)
{
    ExternalPdfTranslateCommand cmd;
    if (!ensureReady())
        primePythonCache(false);

    cmd.python = cachedPython().isEmpty() ? resolvePythonPath() : cachedPython();
    cmd.script = findPdfEnginesScript();
    cmd.args = {
    ...
    cmd.extraEnv.insert(QStringLiteral("ETE_PDF2ZH_TIMEOUT"),
                        QString::number(kExternalPdfTranslateTimeoutSec));
    if (request.runtime == QStringLiteral("cloud") && !request.cloudKey.isEmpty()) {
        cmd.extraEnv.insert(QStringLiteral("OPENAI_API_KEY"), request.cloudKey);
        if (!request.cloudBase.isEmpty())
            cmd.extraEnv.insert(QStringLiteral("OPENAI_BASE_URL"), request.cloudBase);
        if (!request.model.isEmpty())
            cmd.extraEnv.insert(QStringLiteral("OPENAI_MODEL"), request.model);
    }

    return cmd;
}

ExternalPdfTranslateResult DocumentLoader::parseExternalPdfTranslateOutput(
    const ExternalPdfTranslateRequest& request,
    const QByteArray& stdoutOut,
    const QByteArray& stderrOut,
    int exitCode,
...
ExternalPdfTranslateResult DocumentLoader::translatePdfExternal(const ExternalPdfTranslateRequest& request)
{
    ...
    const ExternalPdfTranslateCommand cmd = buildExternalPdfTranslateCommand(request);
    ...
    QByteArray stderrOut;
    int exitCode = -1;
    if (!runPythonScript(cmd.python, cmd.script, cmd.args, kExternalPdfTranslateTimeoutMs, &stdoutOut,
                         &stderrOut, &exitCode, cmd.extraEnv)) {
        result.error = QStringLiteral("Не удалось запустить внешний PDF-движок");
        return result;
    }

    return parseExternalPdfTranslateOutput(request, stdoutOut, stderrOut, exitCode, true);
}
