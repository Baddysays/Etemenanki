...
Model is normally bundled with the installer.
Commands: status, serve, download, install-embedded
"""
...
def cmd_download(app_root: Path) -> int:
    """Recovery download when the bundled model is missing."""
    manifest = load_manifest(app_root)
    model = default_model(manifest)
    if not model:
...
def cmd_install_embedded(app_root: Path) -> int:
    """pip huggingface_hub + download GGUF (recovery path for Settings)."""
    emit_phase("pip")
    emit_progress(0)
    py = sys.executable
...
def _start_server_process(app_root: Path, model_path: Path, manifest: dict) -> subprocess.Popen | None:
    ...
    log_path = log_dir / "embedded-llm-server.log"
...
def cmd_serve(app_root: Path) -> int:
    ...
    if not path.is_file():
        ...
        print("Use Settings → Download built-in model, or reinstall the full setup.", file=sys.stderr)
        return 3
    try:
    ...
    for _ in range(45):
        ...
        if port_open(host, port):
            ...
            return 0
    print(
        f"Server did not start on port {port}. See logs/embedded-llm-server.log",
        file=sys.stderr,
    )
    return 4
