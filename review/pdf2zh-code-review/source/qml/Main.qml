...
ApplicationWindow {
    ...
    minimumHeight: 760
    visible: true
    title: "Etemenanki — AI-переводчик документов"
    color: "#f3f5f9"
    ...
    readonly property color cText: "#1f2b3f"
    readonly property color cMuted: "#6a7892"
    readonly property color cBorder: "#d9e1ef"
    readonly property color cCard: "#ffffff"
    readonly property color cAccent: "#2f78ee"
    readonly property color cOk: "#27a85a"
    ...
    FileDialog {
        ...
        nameFilters: backend.isPdf && backend.hasTranslatedPdf
            ? ["PDF (*.pdf)", "Текст (*.txt)", "Все файлы (*)"]
            : backend.workflowRoundTrip
              ? ["Формат оригинала (*.*)", "Текст (*.txt)", "Все файлы (*)"]
              : ["Текст (*.txt)", "Word (*.docx)", "Markdown (*.md)", "Все файлы (*)"]
    ...
    function remainingTimeLabel() {
        ...
        if (sec <= 0)
            return "Осталось: …"
        return "Осталось: ~" + formatTime(sec)
    }
    ...
    component PrimaryButton: Button {
        ...
        background: Rectangle {
            radius: 10
            color: btn.down ? "#2769d3" : (btn.hovered ? "#3a84f4" : cAccent)
        }
        contentItem: Label {
            horizontalAlignment: Text.AlignHCenter
            verticalAlignment: Text.AlignVCenter
    ...
    component DocPanel: Card {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Label {
                    text: currentPage + " / " + effectivePageCount + " стр."
                    color: cMuted
                    font.pixelSize: 12
                }
            }
    ...
    ColumnLayout {
        ...
        Card {
            ...
            RowLayout {
                ...
                Image {
                    ...
                    ToolTip.text: "Etemenanki — Нет легкого пути от земли к звездам"
                    MouseArea {
                        id: brandLogoMa
                        anchors.fill: parent
                        hoverEnabled: true
                ...
                ColumnLayout {
                    ...
                    Label {
                        text: "Нет легкого пути от земли к звездам"
                        font.pixelSize: 12
                        color: cAccent
                    }
                }
                ...
                RowLayout {
                    ...
                    IconBtn {
                        iconGlyph: "?"
                        ToolTip.visible: hovered
                        ToolTip.text: settings.uiText("help")
        ...
        Card {
            ...
            RowLayout {
                ...
                Label { text: settings.uiText("source_lang"); color: cMuted; font.pixelSize: 12 }
                LangPicker {
                    id: srcLang
                    Layout.preferredWidth: 196
                ...
                Label { text: settings.uiText("target_lang"); color: cMuted; font.pixelSize: 12 }
                LangPicker {
                    id: dstLang
                    Layout.preferredWidth: 196
                ...
                OutlineButton {
                    text: settings.uiText("load_file")
                    iconText: "↑"
                    enabled: !backend.busy
                    onClicked: openDialog.open()
                }
                PrimaryButton {
                    text: settings.uiText("translate")
                    enabled: !backend.busy
        ...
        RowLayout {
            ...
            DocPanel {
                ...
                Layout.fillWidth: true
                Layout.fillHeight: true
                panelTitle: "Просмотр исходного файла"
                fileLabel: backend.fileName || "файл не выбран"
                ...
                textArea.placeholderText: usePdfPreview
                    ? ""
                    : (backend.fileName.length > 0 ? "" : "Выберите файл или вставьте текст")
            }
            ...
            DocPanel {
                ...
                Layout.fillWidth: true
                Layout.fillHeight: true
                panelTitle: "Просмотр переведённого файла"
                ...
                textArea.placeholderText: {
                    if (resultViewMode === "format" && backend.isPdf && !backend.hasTranslatedPdf) {
                        if (backend.busy)
                            return "Сборка PDF с сохранением формата..."
                        return "PDF не собран. Проверьте Python в tools/python_path.txt\nили нажмите «Исходный формат» ещё раз."
                    }
                    if (resultViewMode === "format" && !resultFormatAvailable())
                        return "Форматированный вид появится после перевода"
                    return "Здесь появится перевод после нажатия «Перевести»"
                }
            }
        }
            ...
            RowLayout {
                ...
                RowLayout {
                    ...
                    SegmentBtn {
                        ...
                        implicitWidth: 168
                        text: "Исходный формат (" + sourceFormatLabel() + ")"
                ...
                RowLayout {
                    ...
                    Repeater {
                        model: [
                            { icon: "⏱", title: "Скорость", value: backend.modelSpeed },
                            { icon: "🏅", title: "Качество", value: backend.modelQuality },
                            { icon: "🧠", title: "RAM", value: backend.modelRamNeed },
                            { icon: "🎮", title: "VRAM", value: backend.modelVramNeed }
                        ]
                        delegate: RowLayout {
                            spacing: 8
                            ...
                            ColumnLayout {
                                spacing: 0
                                Label { text: modelData.title; color: cMuted; font.pixelSize: 11 }
                                Label {
        ...
        Card {
            ...
            RowLayout {
                ...
                Label {
                    ...
                    text: "Прошло: " + formatTime(elapsedSec)
                    color: cMuted
                    font.pixelSize: 12
                }
                Label {
                ...
                Item {
                    ...
                    Label {
                        anchors.centerIn: parent
                        text: backend.busy ? (backend.progress + "%") : (backend.progress >= 100 ? "Готово" : "")
                        color: "#52627d"
                        font.weight: Font.DemiBold
                        font.pixelSize: 11
                    }
    ...
    Dialog {
        ...
        property bool helpEn: settings.appUiLanguage === "en"
        ...
        contentItem: ScrollView {
            ...
            ColumnLayout {
                width: helpDialog.width - 48
                spacing: 14

                Label {
                    Layout.fillWidth: true
                    wrapMode: Text.WordWrap
                    text: helpDialog.helpEn
                ...
                Label {
                    text: helpDialog.helpEn ? "Quick start" : "Быстрый старт"
                    font.pixelSize: 15
                    font.weight: Font.Bold
                    color: cText
                }
                Label {
                    ...
                    wrapMode: Text.WordWrap
                    text: helpDialog.helpEn
                          ? "1. Click «Upload file» and choose a document.\n"
                            + "2. Select source and target languages and an AI model.\n"
                            + "3. Click «Translate» and wait for completion.\n"
                            + "4. Use «Save» to export the result (PDF, DOCX, TXT, etc.).\n"
                            + "5. For PDF, switch to «Original format» after translation."
                          : "1. Нажмите «Загрузить файл» и выберите документ.\n"
                            + "2. Укажите языки и модель ИИ.\n"
                            + "3. Нажмите «Перевести» и дождитесь завершения.\n"
                            + "4. «Сохранить» — экспорт результата (PDF, DOCX, TXT и др.).\n"
                            + "5. Для PDF переключитесь на «Исходный формат» после перевода."
                    color: cText
                    lineHeight: 1.35
                }
                ...
                Label {
                    text: helpDialog.helpEn ? "Supported formats" : "Поддерживаемые форматы"
                    font.pixelSize: 15
                    font.weight: Font.Bold
                    color: cText
                }
                ...
                GridLayout {
                    ...
                    Repeater {
                        model: helpDialog.helpEn ? [
                            ["PDF", "Layout-preserving translation and PDF rebuild"],
                            ["DOCX", "Word paragraphs → DOCX or TXT"],
                            ["XLSX / CSV", "Spreadsheet cells → same format"],
                            ["SRT / ASS / VTT", "Subtitle cues → same format"],
                            ["JSON", "String fields → JSON"],
                            ["HTML", "Page text → HTML or TXT"],
                            ["EPUB", "Chapters → EPUB or TXT"],
                            ["TXT / MD", "Plain text and Markdown"]
                        ] : [
                            ["PDF", "Перевод с сохранением вёрстки и сборка PDF"],
                            ["DOCX", "Параграфы Word → DOCX или TXT"],
                            ["XLSX / CSV", "Ячейки таблицы → тот же формат"],
                            ["SRT / ASS / VTT", "Реплики субтитров → тот же формат"],
                            ["JSON", "Строковые поля → JSON"],
                            ["HTML", "Текст страницы → HTML или TXT"],
                            ["EPUB", "Главы → EPUB или TXT"],
                ...
                Label {
                    text: helpDialog.helpEn ? "Runtime modes" : "Режимы работы"
                    font.pixelSize: 15
                    font.weight: Font.Bold
                    color: cText
                }
                Label {
                    ...
                    wrapMode: Text.WordWrap
                    text: helpDialog.helpEn
                          ? "• Local — Ollama on this PC (ollama serve).\n"
                            + "• Cloud — OpenAI-compatible API; configure Base URL and API Key in Settings."
                          : "• Локально — Ollama на этом компьютере (ollama serve).\n"
                            + "• Облако — OpenAI-совместимый API; укажите Base URL и API Key в настройках."
                    color: cText
                    lineHeight: 1.35
                }
                ...
                Label {
                    text: helpDialog.helpEn ? "PDF engines" : "Движки PDF"
                    font.pixelSize: 15
                    font.weight: Font.Bold
                    color: cText
                }
                Label {
                    ...
                    wrapMode: Text.WordWrap
                    text: helpDialog.helpEn
                          ? "Settings → PDF layout: Etemenanki (embedded PyMuPDF) or PDFMathTranslate (portable pdf2zh). "
                            + "Both run locally without servers. See engines/THIRD_PARTY.md for AGPL notices."
                          : "Настройки → Вёрстка PDF: Etemenanki (PyMuPDF) или PDFMathTranslate (портативный pdf2zh). "
                            + "Оба работают локально без серверов. Лицензии — engines/THIRD_PARTY.md."
                    color: cText
                    lineHeight: 1.35
                }
                ...
                Label {
                    text: helpDialog.helpEn ? "Translation settings" : "Настройки перевода"
                    font.pixelSize: 15
                    font.weight: Font.Bold
                    color: cText
                }
                Label {
                    ...
                    wrapMode: Text.WordWrap
                    text: helpDialog.helpEn
                          ? "In Settings → Translation: glossary (term pairs), parallel chunk count, "
                            + "and PDF layout options (auto layout, table preservation)."
                          : "В настройках → Перевод: глоссарий (пары терминов), параллельные фрагменты, "
                            + "опции вёрстки PDF (авто-layout, сохранение таблиц)."
                    color: cText
                    lineHeight: 1.35
                }
            }
    ...
    Component.onCompleted: syncRuntimeFromModels()
}
